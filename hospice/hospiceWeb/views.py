...
def generateJSON(pid):
    ...
    mycursor.execute(
        "SELECT genderId, isCritical FROM `admission` WHERE IPNo = '" + pid + "'")
...
def dataFetchFromSQL(idd):
    ...
    mycursor.execute("SELECT * FROM `gender` WHERE id = '" +
                     str(data[1]) + "'")
    gender = mycursor.fetchone()[1]

    mycursor.execute(
        "SELECT * FROM `disease` WHERE id = '" + str(data[2]) + "'")
    disease = mycursor.fetchone()[1]

    mycursor.execute(
        "SELECT * FROM `previousdisease` WHERE id = '" + str(data[3]) + "'")
    ...
    mycursor.execute(
        "SELECT * FROM `bloodgroup` WHERE id = '" + str(data[5]) + "'")
...
def hospices(request):
    ...
    if not request.user.is_authenticated:
        return redirect('signin')
    IPNo = None
    data = {"ip": all_patient, "submit": "none", "p": p, "patient": "undefined",
            "datediff": "undefined", "chart_name": "undefined"}
    if request.method == 'POST':
        IPNo = request.POST['PId']
        patient, datediff, chart_name = generateJSON(IPNo)

        data, admissionDate, date, name, email, phone, disease, gender = dataFetchFromSQL(
            IPNo)
        ...
        if(len(day(op)) == 2):
            ...
            data = {"admissionDate": str(admissionDate.strftime("%d %b %Y")), "gender": gender, "name": name, "phone": phone, "email": email, "disease": disease, "ip": all_patient, "IPNo": IPNo, "total": total, "price_lab": abs(
                price_lab), "price": abs(price), "date": day(op), "submit": "block", "start": str(start.strftime("%d %b %Y")), "end": str(end.strftime("%d %b %Y")), "p": p, "patient": patient, "datediff": datediff, "chart_name": chart_name}
            return render(request, 'hospiceweb/basic.html', data)
        else:
            start = date + timedelta(days=day(op)[0])

            data = {"admissionDate": str(admissionDate.strftime("%d %b %Y")), "gender": gender, "name": name, "phone": phone, "email": email, "disease": disease, "ip": all_patient, "IPNo": IPNo, "total": total, "price_lab": abs(
                price_lab), "price": abs(price), "date": day(op), "submit": "block", "start": str(start.strftime("%d %b %Y")), "end": "NaN", "p": p, "patient": patient, "datediff": datediff, "chart_name": chart_name}
            return render(request, 'hospiceweb/basic.html', data)
    else:
        return render(request, 'hospiceweb/basic.html', data)
...
def search(request):
    if request.is_ajax():
        queryset = Admission.objects.filter(
            ipno__contains=request.GET.get('search', None))
        lst = []
        for i in queryset:
            lst.append(i.ipno)
        data = {
